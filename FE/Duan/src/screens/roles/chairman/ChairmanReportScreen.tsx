...
export const ChairmanReportScreen = () => {
	...
	const sourceBreakdown = useMemo(() => {
		...
		tasks.forEach(task => {
			if (task.ownerRole === role || task.assigneeRole === role) {
	...
	return (
		<ScrollView contentContainerStyle={styles.screen}>
			<Text style={styles.title}>Báo cáo lãnh đạo</Text>
			<Text style={styles.subtitle}>Số liệu tổng hợp dành cho cấp {roleLabel.toLowerCase()}.</Text>
			...
			<View style={styles.metricRow}>
				...
				<MetricCard label="Hoàn thành" value={`${metrics.completionRate}%`} helper={`${metrics.completed} nhiệm vụ`} accent="#15803D" />
			</View>
			<View style={styles.metricRow}>
				<MetricCard label="Quá hạn" value={String(metrics.overdue)} accent="#DC2626" helper="Cần chỉ đạo" />
				<MetricCard label="Sắp đến hạn" value={String(metrics.dueSoon)} accent="#EA580C" helper="Theo dõi trong 72h" />
			</View>

			<View style={styles.section}>
				<Text style={styles.sectionTitle}>Nguồn giao nhiệm vụ</Text>
				...
				{!Object.keys(sourceBreakdown).length ? <Text style={styles.emptyText}>Chưa có dữ liệu nguồn phù hợp.</Text> : null}
			</View>
			...
			<View style={styles.section}>
				...
				{!departmentRanking.length ? <Text style={styles.emptyText}>Chưa có số liệu để xếp hạng.</Text> : null}
			</View>
		</ScrollView>
	);
};
